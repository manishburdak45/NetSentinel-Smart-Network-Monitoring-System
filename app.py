...
def check_block_status(source_ip, source_mac=None):
    now = time.time()
...
def check_block_verified(method, source_ip, source_mac):
...
def is_ip_currently_blocked(source_ip):
    blocked, method_used, expires_at = check_block_status(source_ip)
    return blocked
...
def remove_extra_incidents():
    if len(incidents) <= MAX_INCIDENTS:
        return
    ordered = sorted(incidents.items(), key=lambda item: item[1]["last_seen"])
    excess = len(incidents) - MAX_INCIDENTS
    for incident_id, record in ordered[:excess]:
        incidents.pop(incident_id, None)
        stale_keys = [key for key, value in incident_index.items() if value == incident_id]
        for stale_key in stale_keys:
            incident_index.pop(stale_key, None)


def count_repeat_attempt(block_method, source_ip, source_mac):
...
def figure_out_response_status(blocked, verification_status, repeat_count):
    if blocked and verification_status == "traffic_stopped":
        return "mitigated" if repeat_count == 1 else "blocked_source_repeated_attempt"
    if blocked:
        return "attack_ongoing"
    return "unblocked"


def build_raw_event(attack_type, alert, blocked, block_method, verification_status, response_status, destination_port, severity, now):
    return {
...
def merge_alert_into_incident(incident, attack_type, alert, raw_event, destination_port, severity, blocked, block_method, block_expires_at, verification_status, response_status, repeat_count, now):
...
def create_new_incident(incident_id, category, attack_type, alert, raw_event, source_ip, target_ip, destination_port, severity, blocked, block_method, block_expires_at, verification_status, response_status, repeat_count, now):
    return {
...
def handle_new_alert(alert):
    ...
    blocked, block_method, block_expires_at = check_block_status(source_ip, source_mac)
    verification_status = check_block_verified(block_method, source_ip, source_mac) if blocked else None

    repeat_count = None
    if blocked:
        repeat_count = count_repeat_attempt(block_method, source_ip, source_mac)
        logger.info(
    ...
    response_status = figure_out_response_status(blocked, verification_status, repeat_count)
    ...
    raw_event = build_raw_event(attack_type, alert, blocked, block_method, verification_status, response_status, destination_port, severity, now)
    ...
    with state_lock:
        ...
        if incident and now - incident["last_seen"] <= INCIDENT_MERGE_WINDOW:
            merge_alert_into_incident(incident, attack_type, alert, raw_event, destination_port, severity, blocked, block_method, block_expires_at, verification_status, response_status, repeat_count, now)
        else:
            incident_id = str(uuid.uuid4())
            incident = create_new_incident(incident_id, category, attack_type, alert, raw_event, source_ip, target_ip, destination_port, severity, blocked, block_method, block_expires_at, verification_status, response_status, repeat_count, now)
            incidents[incident_id] = incident
            incident_index[index_key] = incident_id
            remove_extra_incidents()
...
def update_device_info(info):
    if not isinstance(info, dict) or not info.get("ip"):
        return
    device_key = info.get("mac") or info["ip"]
    with state_lock:
        ...
        devices[device_key] = existing


def remove_extra_scan_jobs():
    ...
    for job_id, job in finished[:to_remove]:
...
def discovery_scan_job(job_id):
    try:
        ...
        if not isinstance(results, list):
            results = []
        with state_lock:
            for found_device in results:
                update_device_info(found_device)
...
def target_scan_job(job_id, target):
    try:
        result = scanner.scan_target(target)
        if not isinstance(result, dict):
            result = {}
        with state_lock:
            update_device_info(result)
            job = scan_jobs.get(job_id)
            if job:
                job["status"] = "completed"
                job["completed_at"] = time.time()
...
def start_scan(job_type, target=None):
    ...
    if job_type == "discovery":
        thread = threading.Thread(target=discovery_scan_job, args=(job_id,), daemon=True)
    else:
        thread = threading.Thread(target=target_scan_job, args=(job_id, target), daemon=True)
    thread.start()
    return job_id
...
def api_devices():
    ...
    result.sort(key=lambda device: device.get("last_seen", 0), reverse=True)
...
def api_scan():
    ...
    if target is not None:
        validated = is_valid_ip_or_network(target)
    ...
    try:
        if validated_target is not None:
            job_id = start_scan("target_scan", validated_target)
        else:
            job_id = start_scan("discovery")
    except Exception:
        scan_lock.release()
...
def api_alerts():
    ...
    with state_lock:
        ...
        ordered = sorted(incidents.values(), key=lambda incident: incident["last_seen"], reverse=True)
        rows = []
        for incident in ordered[:limit]:
            rows.append({
                "id": incident["incident_id"],
                "source_ip": incident["source_ip"],
                "source_mac": incident["source_mac"],
                "target_ip": incident["target_ip"],
                "target_mac": incident["target_mac"],
...
def api_suspicious_sources():
    with state_lock:
        ...
        for incident in ordered:
            ...
            row["first_seen"] = min(row["first_seen"], incident["first_seen"])
            if incident["last_seen"] > row["last_seen"]:
                row["last_seen"] = incident["last_seen"]
            if severity_rank(incident["severity"]) > severity_rank(row["severity"]):
    ...
    for source_ip in source_ips:
        ...
        blocked, method, expires_at = check_block_status(source_ip, row.get("source_mac"))
        row["blocked"] = blocked
        row["block_method"] = method
        row["block_expires_at"] = expires_at
        row["verification_status"] = check_block_verified(method, source_ip, row.get("source_mac")) if blocked else None
        rows.append(row)
...
def parse_ip_list(ips):
    ...
    return unique_ips, failed_ips


def get_mac_hints_for_ips(unique_ips):
    with state_lock:
        interface = monitor_state.get("interface")
        mac_hints = {}
        for incident in incidents.values():
            ip = incident.get("source_ip")
            if ip in unique_ips and incident.get("source_mac") and ip not in mac_hints:
                mac_hints[ip] = incident["source_mac"]
    return interface, mac_hints


def save_block_bookkeeping(method, ip, mac, duration, now):
...
def block_one_ip(ip, mac_hints, interface, duration, now, failed_ips, already_blocked_ips, successfully_blocked):
    try:
    ...
    expires_at = save_block_bookkeeping(method, ip, mac, duration, now)
...
def get_block_summary_message(successfully_blocked, already_blocked_ips, failed_ips):
...
def api_response_block():
    ...
    unique_ips, failed_ips = parse_ip_list(ips)
    ...
    interface, mac_hints = get_mac_hints_for_ips(unique_ips)
    ...
    for ip in unique_ips:
        block_one_ip(ip, mac_hints, interface, duration, now, failed_ips, already_blocked_ips, successfully_blocked)

    success, message = get_block_summary_message(successfully_blocked, already_blocked_ips, failed_ips)
